...
interface RaydiumPoolCardProps {
  pool: {
    name: string;
    platform: string;
    tvl: number;
...
export function RaydiumPoolCard({ pool }: RaydiumPoolCardProps) {
  const [isHovered, setIsHovered] = useState(false);

  // Memoizing tokens and features to avoid unnecessary re-renders
  ...
  return (
    ...
    >
      <CardContent className="p-6">
        ...
        <Link href={`/pools/raydium/${pool.name.toLowerCase()}`}>
          <a>
